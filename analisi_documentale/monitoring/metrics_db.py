    ...
    def save(self, agent_metrics: AgentPerformanceMetrics, rag_metrics: RAGPerformanceMetrics):
        ...
        try:
            ...
            with self._get_conn() as conn:
                cursor = conn.execute(
                    """
                    INSERT INTO requests (query, action_type, total_time, llm_time, upload_time)
                    VALUES (?, ?, ?, ?, ?)
                    """,
                    (agent_metrics.query, agent_metrics.action_type, agent_metrics.total_time, agent_metrics.llm_time, agent_metrics.upload_time)
                )
                ...
                if rag_metrics:
                    ...
                    rag_cursor = conn.execute(
                        """
                        INSERT INTO rag_retrieves (request_id, query, total_time, best_score, worst_score, mean_score, chunks_ids)
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                        """,
                        (request_id, rag_metrics.query, rag_metrics.total_time, rag_metrics.best_score, rag_metrics.worst_score, rag_metrics.mean_score, rag_metrics.chunks_ids)
                    )
